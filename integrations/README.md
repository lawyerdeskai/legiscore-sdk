...
| `legiscore.postman_environment.json` | Shared by both. `base_url` and `search_base_url` preset, everything else empty. |
...
The `search` folder points at `{{search_base_url}}`, not `{{base_url}}`: the search product is
served from its own host, which the spec states as a `servers` entry on each of those paths.
...
`document_id`, `case_id`, `search_id`, `documentId`, `upload_url`, `object_name` — ships blank and stays blank,
